...
export default async function EventiPage() {
  ...
  // Divido in in corso+futuri e passati
    const futuri = eventi
    .filter(e => {
    const start = new Date(e.attributes.DataEvento ?? '');
    const end = e.attributes.DataFine ? new Date(e.attributes.DataFine) : start;
    return end >= today;
    })
    ...
    .filter(e => {
    const end = e.attributes.DataFine ? new Date(e.attributes.DataFine) : new Date(e.attributes.DataEvento ?? '');
    return end < today;
    })
  ...
  {futuri.length > 0 && (
      <section className="pt-4 pb-4 sezione-prossimi-eventi">
        <div className="container">
          <h2 className="mb-4">Eventi in corso/in programma</h2>
          <div className="row text-center">
            {futuri.slice(0, 3).map((evento) => {
              const { Titolo, DataEvento, slug, MainImage } = evento.attributes;
              return (
                <div key={evento.id} className="col-md-4 mb-4 sezione-eventi-passati">
